<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:138:"F:\xampp-win32-5.6.3-0-VC11-installerroot\xammp\htdocs\www\www.backend.devp\backend\public/../application/backend\view\line\line_list.html";i:1550720022;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<nav class="breadcrumb">
	...
	<div class="page-container">
		<form action="<?php echo url('line/line_list'); ?>" method="post">
			<div class="text-c">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a href="javascript:;"
				onclick="line_add('添加案例','<?php echo url('line/line_add'); ?>','700','500')"
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">
				<thead>
				 ...
				 <th scope="col" colspan="14" rowspan="1">注：所有案例列表
				 	<span style="margin-left:13.3%;">统计合集链接：&nbsp;<a href="<?php echo $domain; ?><?php echo url('all/index'); ?>" target="_blank" style="color:#5a98de;"><?php echo $domain; ?><?php echo url('all/index'); ?></a></span>
				 </th>
				 </tr>
					<tr class="text-c">
						<th width="80">ID</th>
				...
				<tbody>
					...
					<tr class="text-c">
						...
						<td class="td-manage">
							<?php if($vo['status']==2): ?> <a style="text-decoration: none"
								onClick="line_start(this,'<?php echo $vo['id']; ?>')" href="javascript:;" title="启用">
								<i class="Hui-iconfont">&#xe6e1;</i></a>
							  <?php else: ?>
							  <a style="text-decoration: none" onClick="line_stop(this,'<?php echo $vo['id']; ?>')" href="javascript:;" title="停用">
							  <i class="Hui-iconfont">&#xe631;</i></a>
							  <?php endif; ?>
							  <a title="编辑" href="javascript:;" onclick="line_edit('修改案例','<?php echo url('line/line_add'); ?>','<?php echo $vo['id']; ?>','700','500')"
								class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe6df;</i>
								</a> 
								<a title="删除" href="javascript:;" onclick="line_del(this,'<?php echo $vo['id']; ?>')"
								class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe6e2;</i></a>
								<a data-title="服务器列表" href="javascript:;" data-href="<?php echo url('server/server_list'); ?>?n=<?php echo $vo['name']; ?>&line_id=<?php echo $vo['id']; ?>" onclick="Hui_admin_tab(this)"
								class="ml-5" style="text-decoration: none">服务器</a>
								<a data-title="落地域名列表" href="javascript:;" data-href="<?php echo url('luodi/luodi_list'); ?>?n=<?php echo $vo['name']; ?>&line_id=<?php echo $vo['id']; ?>" onclick="Hui_admin_tab(this)"
	...
	function line_start(obj,id){
		layer.confirm('确认要启用吗？',function(index){
			$.ajax({
				type: 'POST',
				url: '<?php echo url("line/switch_state"); ?>',
				dataType: 'json',
				data:{"id":id,"status":1},
	...
	function line_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				url: '<?php echo url("line/line_del"); ?>',
				dataType: 'json',
				data:{"ids":id},
				success: function(data){
					if(data==1){
