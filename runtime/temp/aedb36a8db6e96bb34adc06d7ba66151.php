...
<nav class="breadcrumb">
	...
	<div class="page-container">
		<form action="<?php echo url('domain/domain_list'); ?>" method="post">
			<div class="text-c">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a href="javascript:;" onclick="domain_add('添加','<?php echo url('domain/domain_add'); ?>','700','500')"
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">				
				...
				<tbody>
					...
					<tr class="text-c">
					     <td><input type="checkbox" name="checkbox[]" id="<?php echo $vo['id']; ?>"></td>
						<td><?php echo $vo['name']; ?></td>		
					    <td><?php echo $vo['group_name']; ?></td>			
						<td><?php echo $vo['desc']; ?></td>
						<td><?php echo $vo['relate_ip']; ?></td>											
						<td><?php echo !empty($vo['update_time'])?$vo['update_time']:'-'; ?></td>
						<td><?php if($vo['is_ban']==2): ?><span class="label label-success radius">正常</span><?php else: ?><span class="label radius">被封</span><?php endif; ?></td>											
						<td class="td-manage">							
							  <a title="编辑" href="javascript:;" onclick="domain_edit('修改域名','<?php echo url('domain/domain_add'); ?>','<?php echo $vo['id']; ?>','700','500')"
								class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe6df;</i>
								</a> 
								<a title="删除" href="javascript:;" onclick="domain_del(this,'<?php echo $vo['id']; ?>')"
								class="ml-5" style="text-decoration: none">
	...
	function domain_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				url: '<?php echo url("domain/domain_del"); ?>',
				dataType: 'json',
				data:{"ids":id},
