...
<nav class="breadcrumb">
	...
	<div class="page-container">
		<form action="<?php echo url('domain/group_list'); ?>" method="post">
			<div class="text-c">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a href="javascript:;" onclick="group_add('添加','<?php echo url('domain/group_add'); ?>','700','500')"
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">				
				...
				<tbody>
					...
					<tr class="text-c">
						...
						<td><?php echo $vo['mobile_group_name']; ?></td>
						<td><?php echo $vo['desc']; ?></td>									
						<td><?php echo !empty($vo['update_time'])?$vo['update_time']:'-'; ?></td>											
						<td class="td-manage">							
							  <a title="编辑" href="javascript:;" onclick="group_edit('修改分组','<?php echo url('domain/group_add'); ?>','<?php echo $vo['id']; ?>','700','500')"
	...
	function group_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				url: '<?php echo url("domain/group_del"); ?>',
				dataType: 'json',
				data:{"ids":id},
